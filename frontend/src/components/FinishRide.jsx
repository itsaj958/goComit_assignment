...
const FinishRide = (props) => {
    ...
    async function endRide() {
        try {
            // Fetch ride to get tripId created by v1 accept
            const rideId = props.ride?.id || props.ride?._id
            if (!rideId) return
            ...
            if (!tripId) return
            const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/v1/trips/${tripId}/end`, {}, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
            })
